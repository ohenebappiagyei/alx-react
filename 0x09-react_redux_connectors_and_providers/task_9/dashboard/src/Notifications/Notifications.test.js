...
const mockProps = {
  ...
  setNotificationFilter: jest.fn(), // Add the setNotificationFilter mock function
};
...
describe("Notification tests", () => {
  ...
  it("calls setNotificationFilter with URGENT when first button is clicked", () => {
    const wrapper = shallow(<Notifications {...mockProps} />);
    wrapper.find('button').at(1).simulate('click'); // First button after the close button
    expect(mockProps.setNotificationFilter).toHaveBeenCalledWith("URGENT");
  });

  it("calls setNotificationFilter with DEFAULT when second button is clicked", () => {
    const wrapper = shallow(<Notifications {...mockProps} />);
    wrapper.find('button').at(2).simulate('click'); // Second button after the close button
    expect(mockProps.setNotificationFilter).toHaveBeenCalledWith("DEFAULT");
  });
});
